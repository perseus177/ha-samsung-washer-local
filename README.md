...
so numbering alone would not have put them together. That includes the start button at 7,
which is an action rather than a setting — the sequence reads better than the taxonomy does.
...
| `button` Start / resume | Writes `Run`: resumes a paused cycle, or runs whatever the **dial** is set to. The only way to resume |
| `button` Start selected programme | Sends the chosen programme *and* the start. Refuses unless the appliance is idle, since resuming would silently ignore the choice |
| `button` Pause, Cancel | |
